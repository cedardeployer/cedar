...
def policy_custom(state, client, name):
    try:
        # "arn:aws:iam::aws:policy/AdministratorAccess"
        # "arn:aws:iam::734407909462:policy/24hr_AllowMFA_SET"
        ipolicy = client.get_policy(PolicyArn="arn:aws:iam::aws:policy/%s" % (name))
        return ipolicy['Policy']
    except ClientError as e:
        return False
    return False


def role_exists(state, client, name):
    try:
        irole = client.get_role(RoleName=name)
        return irole['Role']
    except ClientError as e:
        return False
    return False
...
def cr_iam_role(state, module, client, name=None, resource=None, policy=None, actionPolicyNames=[], description=''):
    ...
    if actionPolicyNames not in policy_list and len(actionPolicyNames) > 0:
    ...
    try:
        ...
        if "already exists" in e.response['Error']['Message']:
            ...
            try:
                ...
                if overwrite_oldest_policy_versions:
                    # Remove oldest version and try again (AWS limit of 5 versions)
                    versions = client.list_policy_versions(PolicyArn=f'arn:aws:iam::{acct}:policy/{name}')
                    client.delete_policy_version(PolicyArn=f'arn:aws:iam::{acct}:policy/{name}', VersionId=versions['Versions'][-1]['VersionId'])
                    client.create_policy_version(PolicyArn=f'arn:aws:iam::{acct}:policy/{name}', PolicyDocument=actionPolicy, SetAsDefault=True)
                else:
                    raise
            found = True
        else:
