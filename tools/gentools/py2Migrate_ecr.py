...
class MigrationECR:
    ...
    def __init__(self, ecr_from, ecr_to, verbose=False, debug=False):
    ...
    def ecr_last_pushed(self, ecr: Repository, imgName):
        aconnect = ecr.aconnect
        ecr_client = aconnect.__get_client__("ecr")
        response = ecr_client.describe_images(repositoryName=imgName)
        # if 'policy' in imgName:
        #     print(response)
        #     raise Exception("policy")
        if len(response['imageDetails']) > 0:
            for img in response['imageDetails']:
                if 'imageTags' in img:
                    if 'latest' in img['imageTags']:
    ...
    def ecs_get_running_services(self, cluster_name, nextToken=None):
        aconnect = self.ecr_to.aconnect
        ecs_client = aconnect.__get_client__("ecs")
        response = ecs_client.list_services(cluster=cluster_name, maxResults=100)
        services = response['serviceArns']
        if 'nextToken' in response:
            services = services + self.ecs_get_running_services(cluster_name, nextToken)
        return services
...
if __name__ == '__main__':
    ...
    if not reset_only:
        for img in images:
            ...
            if found:
                ...
                source_pushed = migration.ecr_last_pushed(migration.ecr_from, img)
                target_pushed = migration.ecr_last_pushed(migration.ecr_to, img)
                print(source_pushed)
                print(target_pushed)
                if not source_pushed:
                    continue
                if source_pushed > target_pushed:
                    print("...start migration :%s" % img)
                    print("...migrating")
                    migration.migrate()
                else:
                    print("...skipping")
                    image_skipped.append(img)

                # datetime_source = datetime.strptime(source_pushed,dformat)
                # datetime_target = datetime.strptime(target_pushed, dformat) # 2022-09-14 00:36:19-04:00
                # if datetime_source > datetime_target:
                #     print("...migrating problem?")
                source_pushed = migration.ecr_last_pushed(migration.ecr_from, img)
                target_pushed = migration.ecr_last_pushed(migration.ecr_to, img)
                if source_pushed > target_pushed:
                    print("...migration failed img: %s" % img)
                    image_failed.append(img)
    ...
    if image_failed:
        print("::::Failed images below::::::::::::::::::::::::::::::::")
        print(":::::::::::::::::::::::::::::::::::::::::::::::::")
        print("...migration failed images: %s" % image_failed)
    if image_skipped:
        print("::::Skipped images below::::::::::::::::::::::::::::::::")
        print(":::::::::::::::::::::::::::::::::::::::::::::::::")
        print("...migration skipped images: %s" % image_skipped)
