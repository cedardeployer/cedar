...
from microFront import CloudFrontMolder, config_updateRestricted
...
if __name__ == "__main__":
    ...
    print("  ..... INIT..... 0002")
    accID = acctPlus = origin['account']
    if 'sharedas' in origin:
        acctPlus = accID + origin['sharedas']
    region = 'us-east-1'
    awsconnect.stsClient_init()
    sts_client = awsconnect.stsClient
    aconnect = awsConnect(
        acctPlus, origin['eID'], origin['role_definer'], sts_client, region)
    aconnect.connect()
